...
exports.createStripeCheckoutSession = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ success: false, message: 'Server error creating Stripe session.', error: err.message });
  }
};
...
exports.createPayPalOrder = async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, orderId: mockOrderId, approvalUrl: mockApprovalUrl, message: "PayPal order created (mock)." });
        ...
        console.error("❌ Error:", err);
    res.status(500).json({ success: false, message: 'Server error creating PayPal order.', error: err.message });
  }
};
...
exports.updateUserPlan = async (req, res) => {
  ...
  try {
    ...
    const updateQuery = `
      UPDATE users
      SET plan_type = $1, paypal_subscription_id = $2, paypal_payer_id = $3
      WHERE id = $4 RETURNING id, email, plan_type, restaurant_name;
    `;
    ...
    res.json({ success: true, message: 'User plan updated successfully!', updatedUser: result.rows[0] });
        ...
        console.error("❌ Error:", err);
    res.status(500).json({ success: false, message: 'Server error updating user plan.', error: err.message });
  }
};
