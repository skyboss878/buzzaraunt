...
exports.updateDeliverySettings = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ success: false, message: 'Server error updating delivery settings.', error: err.message, error: err.message });
  }
};
...
exports.getDeliverySettings = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ success: false, message: "Server error.", error: err.message, error: err.message, error: err.message });
        console.error("❌ Error:", err);
    res.status(500).json({ success: false, message: 'Server error fetching delivery settings.', error: err.message, error: err.message });
  }
};
