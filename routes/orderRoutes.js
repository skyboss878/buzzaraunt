...
router.post('/', orderController.createOrder); // No verifyToken for customer checkout

// Route to get all orders for a logged-in restaurant owner
router.get('/', verifyToken, checkPlan(["basic", "pro", "enterprise"]), orderController.getUserOrders);

// Route to get a specific order's status (for tracking page)
router.get('/status', orderController.getOrderStatus); // No verifyToken for public tracking link
